...
```{r}
name_file <- "AMNC_QAQ_April.csv" #This is what you should edit each month to change the name of the file
...
```{r}
#using a dummy file and path to test
drive_download(
  "~/testfolder/fakemaster.csv",
  overwrite = TRUE
)

#binding the new month's data to master and creating new master csv file
fm <- read_csv("fakemaster.csv")
master <- bind_rows(fm, quantaqc)
write_csv(master, file ="fakemaster.csv")

#uploading new updated master file to google drive
newmaster <- drive_upload("fakemaster.csv", type = "csv", 
                  path = "/testfolder/fakemaster.csv", overwrite = TRUE)
