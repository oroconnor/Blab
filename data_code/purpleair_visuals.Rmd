...
```{r}
purpdf <- read_csv("april2021_clean.csv")

purpfeath <- read_feather("april2021_clean.feather")
```
...
p1 <- ggplot(purpdf, aes(time,y)) +
        geom_point(aes(y = `PM2.5_ATM_ug/m3_avg`), size= .3, color = "red", alpha =.4) +
        labs(
          x = NULL,
...
p2 <- ggplot(purpdf, aes(time,y)) +
          geom_point(aes(y = `PM1.0_ATM_ug/m3_avg`), size= .3, color = "blue", alpha =.4) +
          labs(
            x = NULL,
            y = expression(Mass - (μg/~m^3)),
...
p3 <- ggplot(purpdf, aes(time,y)) +
        geom_point(aes(y = `PM10_ATM_ug/m3_avg`), size= .3, color = "green", alpha =.4) +
          labs(
          x = NULL,
          y = expression(Mass - (μg/~m^3)),
            subtitle = paste("PM10")
...
b4 <- ggplot(purpdf,aes(x="",y=`PM1.0_ATM_ug/m3_avg`))+geom_boxplot()
b5 <- ggplot(purpdf,aes(x="",y=`PM2.5_ATM_ug/m3_avg`))+geom_boxplot()
b6 <- ggplot(purpdf,aes(x="",y=`PM10_ATM_ug/m3_avg`))+geom_boxplot()
...
hpurpdf <- purpdf %>%
  ...
  summarise(daily_mean = mean(`PM2.5_ATM_ug/m3_avg`), #daily pm2.5 mean
            sdpm2.5 = sd(`PM2.5_ATM_ug/m3_avg`), #standard deviation of daily pm2.5
            sepm2.5 = (sdpm2.5)/sqrt(1440), #standard error of daily pm2.5
            daily_mean10 = mean(`PM10_ATM_ug/m3_avg`), #daily pm2.5 mean
  ) %>%
  na.omit() %>%
  mutate(
    YMD = ymd(date)
